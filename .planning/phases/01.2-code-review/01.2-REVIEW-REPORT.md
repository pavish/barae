...
**Finding: Properly Centralized**

The `APP_SECRET` environment variable is the single source of truth for all cryptographic operations:

| Location | Usage | Status |
|----------|-------|--------|
| `backend/src/config.ts:16` | Declaration as `Type.String()` | ✓ Correct |
| `backend/src/features/auth/service.ts:20` | Passed to better-auth as `secret` | ✓ Correct |
| All other files | No direct usage | ✓ Correct (delegated) |

**What better-auth uses APP_SECRET for internally:**
- Session cookie signing (HMAC - cookie format: `token.signature`)
- JWT signing for email verification tokens
- JWT signing for password reset tokens
- CSRF token generation

**Verification:** No custom `crypto`, `createHash`, or manual JWT signing exists anywhere in the codebase. All encryption/signing is correctly delegated to better-auth.

### Resend Verification Flow

**Finding: Architecturally Sound**

The custom `/api/v1/auth/resend-verification` endpoint exists because when `requireEmailVerification: false` (non-blocking verification), better-auth disables its built-in `/send-verification-email` endpoint.

**Flow Analysis (routes.ts:11-72):**

```
1. Read session cookie ('better-auth.session_token')
2. Extract token from 'token.signature' format
3. Look up session in database → get userId
4. Look up user → verify email not already verified
5. Call auth.api.sendVerificationEmail() ← Delegates to better-auth
```

**Why this is correct:**
| Aspect | Assessment |
|--------|------------|
| Token generation | ✓ Delegates to `auth.api.sendVerificationEmail()` - does NOT roll its own crypto |
| Token signing | ✓ better-auth signs JWT internally with APP_SECRET |
| Token format | ✓ Same format as signup tokens - `/verify-email` handles both |
| Authorization | ✓ Validates session before allowing resend |
| Email lookup | ✓ Verifies user exists and email not already verified |

**Alternative considered:** Could have used better-auth middleware, but direct session lookup is reasonable for a simple custom endpoint that needs to check `emailVerified` status.

### better-auth Configuration Audit

| Setting | Value | Best Practice |
|---------|-------|---------------|
| `secret` | `fastify.config.APP_SECRET` | ✓ From environment |
| `baseURL` | `FRONTEND_URL` | ✓ For correct email links |
| `trustedOrigins` | `[FRONTEND_URL]` | ✓ CSRF protection |
| `session.expiresIn` | 30 days | ✓ Reasonable with daily refresh |
| `session.updateAge` | 1 day | ✓ Daily token refresh |
| `cookieCache.maxAge` | 5 minutes | ✓ Reduces DB lookups |
| `emailAndPassword.minPasswordLength` | 8 | ✓ Acceptable minimum |
| `emailAndPassword.requireEmailVerification` | false | ✓ Non-blocking by design |
| `emailVerification.sendOnSignUp` | true | ✓ Auto-send on signup |

**Conclusion:** better-auth is configured correctly following security best practices. The custom resend-verification endpoint properly delegates token generation to better-auth's API rather than implementing custom cryptography.
